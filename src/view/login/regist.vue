    ...
    <div class="regist">
      ...
      <Form :rules="ruleRegist" :model="formModel" ref="registForm" :label-width="90" >
        ...
        <FormItem label="教务账号：" prop="eduaccount">
          <Input type="text" v-model="formModel.eduaccount" placeholder="Enter your educational account"></Input>
        </FormItem>
        <FormItem label="教务密码：" prop="edupasswd">
          <Input type="password" v-model="formModel.edupasswd" placeholder="Enter your educational password "></Input>
        </FormItem>
    ...
    methods: {
      handleSubmit() {
        this.$refs.registForm.validate(valid => {
          if (valid) {
            putregist(this.formModel.mail, this.formModel.passwd, this.formModel.username, this.formModel.phone,
                this.formModel
                .eduaccount, this.formModel.edupasswd).then(res => {
                console.log(res)
                if (res.data.code == 200) {
                  
                  this.$Message.success("注册成功,系统将在3秒后自动跳转到登录界面")
                   setTimeout(function () {
